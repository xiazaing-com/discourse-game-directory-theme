...
export default apiInitializer("1.8.0", (api) => {
  ...
  async function hydrateGameCovers() {
    document.querySelectorAll(".game-directory-page .topic-list-item").forEach((item) => {
      if (item.querySelector(".game-topic-card__cover") || item.dataset.gameCoverLoading === "true") {
        return;
      }
      ...
      const request = coverCache.get(topicId) || fetch(`/t/topic/${topicId}.json`, {
      ...
      coverCache.set(topicId, request);
      request.then((src) => {
        ...
        item.dataset.gameCoverLoading = "false";
      });
    });
  }

  function enhanceTopicMedia() {
  ...
  function updatePage(path) {
    const isGameCategory = /^\/c\/5-category(?:\/|$)/.test(path);
    ...
    if (isGameCategory) {
      hydrateGameCovers();
      observer = new MutationObserver(hydrateGameCovers);
  ...
  api.onPageChange((url) => updatePage(url));
});
